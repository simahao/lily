...
- **Mantissa**: 53 bits (52 explicitly stored)
  ...
  科学计数法要求第一位非零，二进制非0及1，因此，二进制的科学计数法是<font color='RED'>$1.b_1b_2...b_n*2^e$</font>。为了扩大有效位数，第一位的1作为固定值（这一位是隐藏值，不需要占用存储位），因此有效位数从52变成53，也就是$b_n$中的n=53
...
针对上面讲述的基础知识，我们通过一些例子进一步加深理解，首先我们看一个半精度浮点数的计算过程：
...
3. 数据库（Oracle）中存储的数值为**932.525**，在C++的客户端，保留两位小数显示为**932.52**，在基于Java后端，Javascript前端的Web应用中，保留两位小数后显示**932.53**，谁显示的是对的？
   ...
   * 整合结果
     ...
     * 尾数：110100100 100 **0011**001100110011001100110011001100110011（52 bit），$9+3+40=52$，按照循环节（0011），第53位是0，所以不会产生四舍五入
