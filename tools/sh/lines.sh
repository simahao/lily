...
function statTwoBranch() {
    ...
    git log --pretty='%aN' | sort -u | while read name; do git log $b1..$b2 --author="$name" --pretty=tformat: --numstat --no-merges | gawk '{ add += $1; subs += $2; loc += $1 - $2 } END { printf "\033[34m|%-16s|\033[0m \033[32m%11s\033[0m \033[34m|\033[0m \033[31m%13s\033[0m \033[34m|\033[0m \033[35m%11s\033[0m \033[34m|\033[0m\n", "'$name'", add, subs, loc }' -; done
    echo -e "\033[34m└────────────────┴─────────────┴───────────────┴─────────────┘\033[0m"
}
